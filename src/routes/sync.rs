...
async fn resolve_sync_path(
    ...
    let canonical_target = target_path.canonicalize().map_err(|e| {
        error!(
            "Sync path '{}' not found or invalid: {}",
            target_path.display(),
            e
        );
        AppError::Io(e)
...
pub async fn get_sync_config_handler(
                    ...
                    if let Ok(relative_path) = path.strip_prefix(&target_path) {
                        if let Ok(mut file) = File::open(path) {
                            ...
                            if std::io::copy(&mut file, &mut hasher).is_ok() {
                                ...
                                manifest
                                    .insert(relative_path.to_string_lossy().replace('\\', "/"), hash);
                            }
                        }
                    }
                }
...
pub async fn sync_code_handler(
    ...
    Ok(Json(
        serde_json::json!({ "message": format!("Sync complete. Wrote {} files.", files_written) }),
    ))
}
